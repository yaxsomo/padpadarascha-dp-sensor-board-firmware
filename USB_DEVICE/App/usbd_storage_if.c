...
#define STORAGE_ACCESS_TIMEOUT_MS        5000U
...
static int8_t STORAGE_WaitForTransfer_HS(void);
...
int8_t STORAGE_GetCapacity_HS(uint8_t lun, uint32_t *block_num, uint16_t *block_size)
{
  ...
  *block_num = sd_info.LogBlockNbr - 1U;
  *block_size = (uint16_t)sd_info.LogBlockSize;
  return (USBD_OK);
  /* USER CODE END 10 */
}
...
int8_t STORAGE_IsReady_HS(uint8_t lun)
{
  ...
  return ((HAL_SD_GetCardState(&hsd2) == HAL_SD_CARD_TRANSFER) ? USBD_OK : USBD_BUSY);
  /* USER CODE END 11 */
}
...
int8_t STORAGE_Read_HS(uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
  ...
  if (HAL_SD_ReadBlocks(&hsd2, buf, blk_addr, blk_len, STORAGE_ACCESS_TIMEOUT_MS) != HAL_OK)
  {
    return (USBD_FAIL);
  }

  return STORAGE_WaitForTransfer_HS();
  /* USER CODE END 13 */
}
...
int8_t STORAGE_Write_HS(uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
  ...
  if (HAL_SD_WriteBlocks(&hsd2, buf, blk_addr, blk_len, STORAGE_ACCESS_TIMEOUT_MS) != HAL_OK)
  {
    return (USBD_FAIL);
  }
...
static int8_t STORAGE_WaitForTransfer_HS(void)
{
  ...
  {
    if ((HAL_GetTick() - start_tick) >= STORAGE_ACCESS_TIMEOUT_MS)
    {
      return (USBD_FAIL);
    }
  }
